...
async function loadMovies() {
  ...
  if (!result || !result.ok) { grid.innerHTML = '<p class="loading-text">Gagal memuat film.</p>'; return; }
  ...
  grid.innerHTML = movies.map(m => `
    <div class="movie-card" onclick="showMovieDetail(${m.id})">
      <div class="movie-poster">
        ${m.poster_url ? `<img class="movie-poster-img" src="${m.poster_url}" alt="${m.title}" onerror="this.parentElement.innerHTML='🎬'"/>` : '🎬'}
      </div>
      <div class="movie-card-info">
        <div class="movie-card-title">${m.title}</div>
        <div class="movie-card-year">${m.year || ''}</div>
        <div class="movie-card-rating">★ ${m.rating || 'N/A'}</div>
      </div>
    </div>
  `).join('');
}
...
async function showMovieDetail(movieId) {
  ...
  if (!result || !result.ok) { container.innerHTML = '<p class="loading-text">Gagal memuat detail.</p>'; return; }
  const m = result.data;
  ...
  container.innerHTML = `
    <div class="detail-poster">
      ${m.poster_url ? `<img src="${m.poster_url}" alt="${m.title}" onerror="this.parentElement.innerHTML='🎬'"/>` : '🎬'}
    </div>
    <div class="detail-info">
      ...
      <div class="detail-cast"><h3>Pemeran</h3><div class="cast-list">${cast}</div></div>
      ${favBtn}
    </div>
  `;
  if (isLoggedIn()) checkIfFavorite(m.id);
}

function searchMovies() {
  const q = document.getElementById('search-input').value.toLowerCase();
  const filtered = allMovies.filter(m => m.title.toLowerCase().includes(q) || (m.genres && m.genres.toLowerCase().includes(q)));
...
async function checkIfFavorite(movieId) {
  ...
  if (result && result.ok && result.data.favorited) {
    ...
    if (btn) { btn.textContent = '♥ Tersimpan'; btn.style.background = 'var(--accent)'; btn.style.color = '#fff'; }
  }
}
...
async function loadFavoriteMovies() {
  ...
  if (!result || !result.ok || result.data.length === 0) {
    grid.innerHTML = `<p class="empty-state">Belum ada favorit. <a href="#" onclick="navigate('movies')">Cari film</a></p>`;
    return;
  }
